...
- **IPアドレス**：`52.197.123.45`みたいな数字の羅列。コンピュータはこれで通信する。
- **ドメイン**：`cupid.click`みたいな人間が覚えやすい名前。

例えば：
- `google.com` → `142.250.196.78`
- `amazon.co.jp` → `54.240.226.134`

⋯⋯数字だと覚えられない。だから、ドメインを使う。

#### DNSって何？

DNS（Domain Name System）は、ドメインをIPアドレスに変換する仕組み。

```
あなた: 「cupid.clickに接続したい」
  ↓
DNS: 「cupid.clickは52.197.123.45だよ」
  ↓
あなた: 「じゃあ52.197.123.45に接続」
```
...
Route 53は、AWSが提供するDNSサービス。

できること：
1. **ドメインの購入**：`cupid.click`みたいなドメインを買える
2. **DNSの管理**：「cupid.clickは52.197.123.45」という情報を世界中に伝える
3. **高速・安定**：世界中にサーバーがあるから、どこからでも速い

⋯⋯ドメインを買って、DNSを管理する、両方できる。

#### なぜRoute 53が必要？

LINE Botは、LINEからWebhookを受け取る。

LINEの要件：
- **HTTPS必須**：`https://`で始まるURL
- **独自ドメイン推奨**：IPアドレス直接は避けるべき

つまり：
- ❌ `http://52.197.123.45/webhook` → HTTPだからダメ
- ❌ `https://52.197.123.45/webhook` → IPアドレス直接は非推奨
- ⭕ `https://cupid.click/webhook` → これが正解

⋯⋯だから、ドメインが必要。Route 53で取得する。

#### Hosted Zoneって何？

Hosted Zoneは、ドメインのDNS設定を管理する場所。

例えば、`cupid.click`のHosted Zoneには：
- **Aレコード**：`cupid.click` → `52.197.123.45`（EC2のIPアドレス）
- **NSレコード**：「このドメインのDNS管理はこのサーバー」という情報
- **SOAレコード**：DNS設定の管理情報

⋯⋯ドメインを買うと、自動的にHosted Zoneが作られる。
...
1. ブラウザで [https://console.aws.amazon.com/](https://console.aws.amazon.com/) を開く
2. AWSアカウントでログイン

##### ステップ2: Route 53サービスを開く

1. 画面上部の検索バーに「Route 53」と入力
2. 検索結果から「Route 53」をクリック

または：
- 左上の「サービス」メニュー → 「ネットワーキングとコンテンツ配信」 → 「Route 53」

##### ステップ3: ドメイン登録画面を開く

1. Route 53のダッシュボード左メニューで「ドメイン」セクションを探す
2. 「登録済みドメイン」をクリック
3. 右上の「ドメインを登録」ボタンをクリック

⋯⋯もし「ドメインの登録」というメニューがあれば、それでもいい。

##### ステップ4: ドメイン名を検索

1. 「ドメイン名を選択」の入力欄に、希望のドメイン名を入力
   - 例：`cupid`、`cupid-bot`、`my-cupid-app`など
2. 「検索」をクリック
...
⋯⋯メールアドレスは確実に受け取れるものを。認証に使う。

3. **プライバシー保護**：
   - 「プライバシー保護を有効にする」：**ONにする**
   - これで、Whois情報にあなたの個人情報が公開されない

##### ステップ8: 利用規約に同意

1. 「Route 53のドメイン登録契約を読み、同意しました」にチェック
2. 「注文を完了」をクリック

##### ステップ9: 支払い完了

1. クレジットカード情報が自動的に使われる
2. 購入が完了すると、メールが届く

⋯⋯購入処理には数分から数時間かかることがある。

#### 1-2. 購入確認メールをチェック

1. 登録したメールアドレスを確認
2. 「Email Address Verification」という件名のメールが届く
3. メール内のリンクをクリックして、**メールアドレスを認証**

⋯⋯このステップを忘れると、ドメインが使えなくなる可能性がある。必ず確認する、ね。

#### 1-3. Hosted Zoneの確認

ドメイン購入が完了すると、自動的にHosted Zoneが作られる。

##### ステップ1: Hosted Zoneを開く

1. Route 53のダッシュボードに戻る
2. 左メニューの「ホストゾーン」をクリック
3. 購入したドメイン（例：`cupid.click`）が表示されることを確認

##### ステップ2: Hosted Zoneの内容を確認

1. ドメイン名をクリック
2. 以下の2つのレコードが自動的に作られていることを確認：

**NSレコード（ネームサーバーレコード）**：
```
タイプ: NS
名前: cupid.click
値: ns-123.awsdns-12.com.
    ns-456.awsdns-45.net.
    ns-789.awsdns-78.org.
    ns-012.awsdns-01.co.uk.
```

⋯⋯これは「このドメインのDNSはAWSが管理してる」という情報。

**SOAレコード（権威情報レコード）**：
```
タイプ: SOA
名前: cupid.click
値: ns-123.awsdns-12.com. awsdns-hostmaster.amazon.com. ...
```

⋯⋯DNSの管理情報。気にしなくていい。

##### ステップ3: Hosted Zone IDを控える（オプション）

- Hosted Zoneのページの右上に「Hosted zone ID: Z1234567890ABC」みたいな表示がある
- これは後で使うかもしれないから、メモしておくといい
...
以下のような情報が表示されればOK：
```
Domain Name: CUPID.CLICK
Registry Domain ID: ...
Registrar: Amazon Registrar, Inc.
...
```

⋯⋯購入直後は反映に時間がかかる。数時間待つこともある。

#### DNSが機能しているか確認

```bash
# ネームサーバーを確認
nslookup -type=NS cupid.click

# または
dig NS cupid.click
```

AWSのネームサーバー（`awsdns-*.com`など）が返ってくればOK。

⋯⋯これも反映に時間がかかることがある。最大48時間、通常は数分から数時間。
...
⋯⋯**ONにしたほうがいい**。
- OFFにすると、Whois情報であなたの名前・住所・電話番号が公開される
- ONにすると、AWSの情報が代わりに表示される
...
- **初年度**：$3.00
- **翌年以降**：$3.00/年（自動更新）
- **Hosted Zone**：$0.50/月（最初の25個は無料期間あり）
...
1. 迷惑メールフォルダを確認
2. 数時間待つ
3. Route 53の「登録済みドメイン」で、ドメインのステータスを確認
   - 「Pending」→まだ処理中
   - 「Email verification required」→メール認証が必要

⋯⋯メールが来ない場合は、AWSサポートに連絡。
...
**対処法**：
- 数時間から最大48時間待つ
- Route 53の「ホストゾーン」で、NSレコードが存在することを確認
...
**対処法**：
1. Route 53ダッシュボードで「ホストゾーン」をリロード
2. 数分待ってみる
3. それでもない場合：
   ```
   「ホストゾーン」→「ホストゾーンの作成」
...
⋯⋯通常は自動で作られる。手動で作ることもできる。

---

### 次のステップへ

⋯⋯以下を確認できたら、Phase 2へ進む、ね。

**チェックリスト**：
- [ ] ドメインを購入できた（例：`cupid.click`）
- [ ] メール認証を完了した
- [ ] Hosted Zoneが作成されている
- [ ] NSレコードとSOAレコードが存在する
- [ ] `whois`コマンドでドメイン情報が見れる（数時間後）

⋯⋯DNS伝搬を待つ間に、Phase 2の準備を始めてもいい、よ。
