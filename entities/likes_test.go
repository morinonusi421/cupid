...
func testLikeToOneUserUsingFromUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.FromUserID, foreign.LineUserID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.FromUser().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadFromUser(ctx, tx, false, (*[]*Like)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadFromUser(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.FromUser == nil {
		t.Error("struct should have been eager loaded")
...
func testLikeToOneSetOpUserUsingFromUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetFromUser(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if x.R.FromUserLike != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.FromUserID, x.LineUserID) {
		...
		if !queries.Equal(a.FromUserID, x.LineUserID) {
			t.Error("foreign key was wrong value", a.FromUserID, x.LineUserID)
		}
	}
}
...
var (
	likeDBTypes = map[string]string{`ID`: `INTEGER`, `FromUserID`: `TEXT`, `ToName`: `TEXT`, `ToBirthday`: `TEXT`, `Matched`: `INTEGER`, `CreatedAt`: `TEXT`}
	_           = bytes.MinRead
)
...
func testLikesSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(likeAllColumns, likePrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, likeGeneratedColumns)
	}
