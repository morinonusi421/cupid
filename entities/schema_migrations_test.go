...
func testSchemaMigrationsExists(t *testing.T) {
	...
	e, err := SchemaMigrationExists(ctx, tx, o.ID)
	if err != nil {
...
func testSchemaMigrationsFind(t *testing.T) {
	...
	schemaMigrationFound, err := FindSchemaMigration(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
var (
	schemaMigrationDBTypes = map[string]string{`ID`: `VARCHAR(255)`, `AppliedAt`: `DATETIME`}
	_                      = bytes.MinRead
)
