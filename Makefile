.PHONY: help build test test-integration generate mocks deploy status logs restart reset-db-local reset-db add-test-user db-copy db-cli db-open
...
reset-db:
	...
	ssh cupid-bot "bash -l -c 'cd ~/cupid && sudo systemctl stop cupid && rm -f cupid.db && sudo systemctl start cupid && sleep 2 && sudo systemctl status cupid'"
