...
- **タイプ**: t4g.small (ARM64)
  - ※ t4g.smallは新規・既存アカウント問わず750時間/月無料トライアル対象（〜2026/12/31）
  - ※ t4g.microは無料枠対象外のため2026-03-04に切り替え済み
...
LINE公式アカウントの管理画面から設定済み。
LINEの機能で、リッチメニューを開くとリンクになった画像のバーが出てくる。
左のボタンからは自分の情報を登録/編集する下記URLへ、右のボタンからは想い人の情報を登録/編集する下記URLへ飛ばされる。
...
### Go
wc2 - **バージョン**: 1.26.0（`go.mod` の `go` ディレクティブで管理）
- **EC2インストール済み**: Go 1.26.0 linux/arm64 (`/usr/local/go/bin/go`)
